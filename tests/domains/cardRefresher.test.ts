...
describe("card refresher tools", () => {
  it("submits a card refresher inquiry", async () => {
    const { transport, calls } = createMockTransport(new Map([["POST /card_refresher/inquiry.json", { data: { inquiry: { token: "inq1" } } }]]));
  ...
  it("shows a card refresher inquiry", async () => {
    ...
    const { transport } = createMockTransport(new Map([["GET /card_refresher/inquiry/inq1.json", { data: inquiry }]]));
    const result = await findTool("spreedly_card_refresher_show_inquiry").handler({ inquiry_token: "inq1" }, { transport });
    expect(result).toEqual(inquiry);
  });

  it("lists card refresher inquiries", async () => {
    const list = { inquiries: [{ token: "inq1" }, { token: "inq2" }] };
    const { transport } = createMockTransport(new Map([["GET /card_refresher/inquiries.json", { data: list }]]));
    const result = await findTool("spreedly_card_refresher_list_inquiries").handler({}, { transport });
