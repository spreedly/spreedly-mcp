...
describe("protection tools", () => {
  it("forwards a claim", async () => {
    const { transport, calls } = createMockTransport(
      new Map([
        [
          "POST /protection/FakeTxToken_xyz789/claims.json",
  ...
  it("shows a protection event", async () => {
    ...
    const { transport } = createMockTransport(
      new Map([["GET /protection/events/ev1.json", { data: event }]]),
    );
    const result = await findTool("spreedly_protection_show_event").handler(
      { event_token: "ev1" },
      { transport },
    );
  ...
  it("creates a protection provider", async () => {
    const { transport, calls } = createMockTransport(
      new Map([
        ["POST /protection/providers.json", { data: { protection_provider: { token: "pp1" } } }],
      ]),
    );
    await findTool("spreedly_protection_create_provider").handler(
      { protection_provider: { type: "test" } },
      { transport },
    );
    expect(calls[0].method).toBe("POST");
  });
  ...
  it("shows a protection provider", async () => {
    ...
    const { transport } = createMockTransport(
      new Map([["GET /protection/providers/pp1.json", { data: provider }]]),
    );
    const result = await findTool("spreedly_protection_show_provider").handler(
      { protection_provider_token: "pp1" },
      { transport },
    );
