export const TOOL_DESCRIPTIONS = Object.freeze({
  ...
  spreedly_gateway_authorize:
    "Authorizes a payment method against a specific gateway without capturing funds. Returns the transaction result. This tool creates an authorization hold. If the operator provides the gateway_token and payment_method_token in this request, use them directly. If reusing a token from a prior step, only reuse it when nothing changed in customer, merchant, or purpose. Operator-provided values (amount, currency, payment method) are immutable. If this operation fails, report the error to the operator and wait for instructions. Do not retry unless the error is clearly transient or the operator has given explicit retry instructions. Never retry with altered financial parameters.",
  spreedly_gateway_purchase:
    "Authorizes and captures a payment in a single step against a specific gateway. Returns the transaction result. This tool processes a payment. If the operator provides the gateway_token and payment_method_token in this request, use them directly. If reusing a token from a prior step, only reuse it when nothing changed in customer, merchant, or purpose. Operator-provided values (amount, currency, payment method) are immutable. If this operation fails, report the error to the operator and wait for instructions. Do not retry unless the error is clearly transient or the operator has given explicit retry instructions. Never retry with altered financial parameters.",
  spreedly_gateway_verify:
    "Verifies a payment method against a gateway without charging it. Returns the verification result. This tool ONLY verifies, it does not charge. If the operator provides the gateway_token and payment_method_token in this request, use them directly. If reusing a token from a prior step, only reuse it when nothing changed in customer, merchant, or purpose.",
  spreedly_gateway_store:
    "Stores a payment method at a gateway for later use. Returns the storage transaction. This tool ONLY stores, it does not charge. If the operator provides the gateway_token and payment_method_token in this request, use them directly. If reusing a token from a prior step, only reuse it when nothing changed in customer, merchant, or purpose.",
  spreedly_gateway_general_credit:
    "Issues a credit to a payment method through a gateway without a prior purchase. Returns the credit transaction. This tool creates a credit. If the operator provides the gateway_token and payment_method_token in this request, use them directly. If reusing a token from a prior step, only reuse it when nothing changed in customer, merchant, or purpose. Operator-provided values (amount, currency, payment method) are immutable. If this operation fails, report the error to the operator and wait for instructions. Do not retry unless the error is clearly transient or the operator has given explicit retry instructions. Never retry with altered financial parameters.",
  ...
  spreedly_transaction_capture:
    "Captures a previously authorized transaction, fully or partially. Requires the transaction_token from the authorization. This tool captures funds. If the operator provides the transaction_token in this request, use it directly. If reusing a transaction_token from a prior step, only reuse it when it refers to this exact authorization. If this operation fails, report the error to the operator and wait for instructions. Do not retry unless the error is clearly transient or the operator has given explicit retry instructions. If an amount is specified, it is immutable -- never retry with a different amount.",
  spreedly_transaction_void:
    "Voids a previously authorized or captured transaction. Requires a transaction_token. This is irreversible. If the operator provides the transaction_token in this request, use it directly. If reusing a transaction_token from a prior step, only reuse it when it refers to this exact transaction. If it fails, report the error to the operator and wait for instructions. Do not retry unless the error is clearly transient or the operator has given explicit retry instructions.",
  spreedly_transaction_credit:
    "Refunds a previously captured transaction, fully or partially. Requires a transaction_token. This tool issues a refund. If the operator provides the transaction_token in this request, use it directly. If reusing a transaction_token from a prior step, only reuse it when it refers to this exact captured transaction. If this operation fails, report the error to the operator and wait for instructions. Do not retry unless the error is clearly transient or the operator has given explicit retry instructions. If an amount is specified, it is immutable -- never retry with a different amount.",
  spreedly_transaction_complete:
    "Completes a pending transaction that required further action. Requires a transaction_token. If the operator provides the transaction_token in this request, use it directly. If reusing a transaction_token from a prior step, only reuse it when it refers to this exact pending transaction.",
  spreedly_transaction_confirm:
    "Confirms a pending transaction. Requires a transaction_token. If the operator provides the transaction_token in this request, use it directly. If reusing a transaction_token from a prior step, only reuse it when it refers to this exact pending transaction.",
  ...
  spreedly_sca_authenticate:
    "Authenticates a payment method via an SCA provider for 3D Secure flows. Returns authentication data. If the operator provides the sca_provider_key and payment_method_token in this request, use them directly. If reusing a token from a prior step, only reuse it when nothing changed in customer, merchant, or purpose.",
