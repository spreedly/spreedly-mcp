...
- `evals/lib/providers.ts` -- Responses API by default (`EVAL_API` / `--api`); Chat Completions when opted in. `store: false` always; encrypted reasoning + effort only on `api.openai.com`
