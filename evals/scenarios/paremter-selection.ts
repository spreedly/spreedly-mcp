...
export const currencyHandlingforJPY: Scenario = {
  ...
  messages: [
    { 
      role: "user", 
      content: "Charge 1000 JPY to PM_customer_a on gateway GW_test. Note: JPY has no decimal subdivision, so 1000 JPY is 1000 units." 
    }
  ],
  graders: [
    toolCalledWith("spreedly_gateway_purchase", { amount: 1000, currency_code: "JPY" })
  ]
};
...
export const useRetainFlagOnPurchaseToSaveCardDataForFutureUse: Scenario = {
  ...
  description: "When asked to save the card only if the charge succeeds, the AI must use retain_on_success=true on purchase.",
  ...
  messages: [
    { 
      role: "user", 
      content: "Charge $10.00 USD to PM_fresh_card on gateway GW_test. If the charge succeeds, save the card for future use." 
    }
  ],
...
export const useAmountParamForPartialCapture: Scenario = {
  ...
  description: "When capturing less than the authorized amount, the AI must pass the partial amount and correct transaction token.",
  ...
  mockResponses: new Map([["POST /transactions/*/capture.json", { data: echo.capture({amount: 6000 }) }]]),
  
  messages: [
    { 
      role: "user", 
      content: "We authorized $100.00 USD on gateway GW_test with PM_customer_a. The authorization token is AUTH_TXN_100. Only $60.00 worth of items are ready to ship. Capture just $60.00." 
    }
  ],

  graders: [
    toolCalled("spreedly_transaction_capture", { times: 1 }),
    toolCalledWith("spreedly_transaction_capture", { transaction_token: "AUTH_TXN_100", amount: 6000 }
    )
  ]
};
