...
export const useVoidInsteadOfCreditForUncapturedAuthorization: Scenario = {
  ...
  mockResponses: new Map([["POST /transactions/*/void.json", { data: echo.void({ token: "AUTH_TXN_001" }) }]]),
  messages: [
    {
      role: "user",
      content:
...
export const useCreditInsteadOfVoidForRefundOfPurchase: Scenario = {
    ...
    mockResponses: new Map([["POST /transactions/*/credit.json", { data: echo.credit({ token: "AUTH_TXN_001"}) }]]),

    messages: [
        {
             role: "user",
             content: "Customer wants a full refund for their completed purchase. Transaction token: TXN_purchase_001. Process the refund." 
        }
    ],
    graders: [
...
export const useVerifyToValidate : Scenario = {
    ...
    description: "When asked to validate a card without charging, the AI must use verify, not authorize or purchase.",
...
export const useGeneralCreditInsteadOfRefundOnGoodwillCredit: Scenario = {
    ...
    description: "When issuing a standalone goodwill credit (not tied to a prior transaction), the AI must use gateway_general_credit, not transaction_credit.",
    ...
    messages: [
        { 
            role: "user", 
            content: "Issue a $25.00 USD goodwill credit to payment method PM_customer_a on gateway GW_test. This is NOT a refund for any previous transaction -- it's a standalone credit." 
        }
    ],
